...
export class InscriptionComponent implements OnInit {
  ...
  constructor(
    ...
    this.registraFormInscription = this.fb.group({
      ...
      tituloSolucion: ['', [Validators.pattern(/^[^a-zA-ZÀ-ÿ\s]{5,50}$/), Validators.required]],
      descripcionSolucion: ['',[Validators.pattern(/^[^a-zA-ZÀ-ÿ\s]{20,2000}$/), Validators.required]],
      nombreEquipo: ['', [Validators.pattern(/^[^a-zA-ZÀ-ÿ\s]{5,50}$/), Validators.required]],
  ...
  addParticipantInscription() {
    if (this.dataSource.length < 5) {
      ...
      this.dialog.open(GeneralDialogComponent, {
        data: {
          message:
            'Ha inscrito el máximo de 5 integrantes. Puede eliminar o modificar',
        },
      });
    }
  }
      ...
      confirmation.afterClosed().subscribe((result) => {
        if (result === true) {
          ...
          const newInscription: Inscription = {
            ...
            descripcionSolucion:
              this.registraFormInscription.value.descripcionSolucion,
          ...
          this.inscriptionService.CreateInscription(newInscription).subscribe(
            (result: any) => {
              ...
              this.dialog.open(GeneralDialogComponent, {
                data: {
                  message:
                    'Ha sido inscrito correctamente. Revise su correo para conocer los siguiente pasos.',
                },
              });
